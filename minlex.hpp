...
private:
  ...
  static SudokuInitializer si;

  unsigned char data[81];
  mbit bdata[9];
  static mbit column_bits[9];
  ...
  // Thanks herumi!
  mbit left_most_bit(mbit v) const {
    union ai {
      uint64_t a[2];
      mbit i;
    } ai;
...
public:
  ...
  Sudoku perm_restrbox2(const int a[3], int pos) {
    mbit bdata2[9] = {};
    mbit mask = ((mbit(1) << 81) - 1) ^ ((mbit(1) << 27) - 1)
                                            << (27 * (1 - pos));
  ...
  Sudoku perm_restrbox(const int ai[3], int aj[3]) {
    mbit bdata2[9];
  ...
  Sudoku perm_columns(const int ai[3], const int aj[3], const int ak[3]) {
    mbit bdata2[9];
    mbit v[9];
    for (int i = 0; i < 9; i++) {
      for (int j = 0; j < 9; j++) {
  ...
  Sudoku perm_toprbox(const int a[3]) {
    mbit bdata2[9];
  ...
  Sudoku perm_cbox(const int a[3]) {
    mbit bdata2[9];
    mbit t2 = 0;
    for (int i = 0; i < 9; i++) {
      t2 |= (mbit(7) << (i * 9));
  ...
  Sudoku perm_rbox(const int a[3]) {
    mbit bdata2[9];
    mbit v[3];
    mbit t0 = (mbit(1) << 27) - 1;
    mbit t1 = t0 << 27;
...
private:
  Sudoku min, max;
  const bool hlb_table[8][6] = {{true, true, true, true, true, true},
  ...
  const int perm3[6][3] = {{0, 1, 2}, {0, 2, 1}, {1, 0, 2},
                           {1, 2, 0}, {2, 0, 1}, {2, 1, 0}};
...
public:
  ...
  void perm_columns(Sudoku &g) {
    ...
    for (int k = 0; k < 6; k++) {
      if (!hlb_table[h3][k])
        continue;
      const int *ak = perm3[k];
      for (int j = 0; j < 6; j++) {
        if (!hlb_table[h2][j])
          continue;
        const int *aj = perm3[j];
        for (int i = 0; i < 6; i++) {
          if (!hlb_table[h1][i])
            continue;
          const int *ai = perm3[i];
  ...
  void perm_cbox(Sudoku &g, int hb_min) {
    for (auto a : perm3) {
      ...
      if (g2.headbox_index(false) > hb_min)
        continue;
...
__attribute__((weak)) mbit Sudoku::column_bits[9];
__attribute__((weak)) Sudoku::SudokuInitializer Sudoku::si;
